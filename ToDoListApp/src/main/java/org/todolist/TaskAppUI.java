...
public class TaskAppUI extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        firstRow.getChildren().addAll(addButton, deleteButton);
...
//    }
    ...
    private void deleteSelectedTask() {
        int selectedIndex = taskListView.getSelectionModel().getSelectedIndex();
        if (selectedIndex >= 0) {
            Task selectedTask = taskManager.getTasks().get(selectedIndex); // Get the task at the selected index
            int taskId = selectedTask.getId(); // Assuming you have a getId() method in the Task class
            ...
            alert.showAndWait().ifPresent(response -> {
                if (response == ButtonType.OK) {
                    taskManager.deleteTask(taskId); // Pass the taskId to the deleteTask method
                    updateTaskList();
...
//    }
    ...
    private void showEditDescriptionDialog(Task taskToEdit) {
        ...
        saveButton.setOnAction(e -> {
            ...
            taskManager.editTask(taskToEdit.getId(), taskToEdit.getTitle(), newDescription, taskToEdit.getDueDate(),
                    taskToEdit.getCategory(), taskToEdit.getPriority(), taskToEdit.isCompleted());
            updateTaskList();
